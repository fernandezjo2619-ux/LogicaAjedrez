...
namespace AjedrezLogica.TiposReglasMovimiento
{
    internal class MovimientosHelp
    {
        public static void AgregarDireccion(
            ...
            int dx, int dy,
            ColorPieza bando,
            Tablero tablero, bool saltaPieza = false, bool puedeComer = true, int alrededores = 0, int limitacionMovimiento = 0)
        {
            int i = 1;
            ...
            while (tablero.EsDentroDelTablero(posicion.x + dx * i, posicion.y + dy * i))
            {
                if (tablero.Grid[posicion.x + dx * i, posicion.y + dy * i].EstaOcupado)
                {
                    ...
                    if ((puedeComer || alrededores == i) && !tablero.Grid[posicion.x + dx * i, posicion.y + dy * i].Ocupante.Color.Equals(bando))
                    {
